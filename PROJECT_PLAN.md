...
**2. KOL 分身**：基于 B 站 UP 主的公开内容，构建个体级数字孪生。（本期暂缓）
...
- [ ] **任务 3：演示脚本** —— 从测试题集 5 个 Sheet 各挑 2-3 题，设计演示路径：选标签→生成画像→提问→溯源→换标签对比。准备 Plan B。产物：演示脚本 + 结课汇报 PPT。

- [ ] **任务 4：全程验收把关** —— 前端出来对照交互稿验收、API 出来对照接口文档验收、画像出来和用研一起看、对话出来用测试题实测。产物：验收记录。
...
- [x] **任务 2：pgvector 安装与环境就绪** —— 从源码编译 pgvector 0.8.5 到 PG16，建 HNSW 索引。CI/CD 模板就绪。~~产物：pgvector 安装记录~~ 🔧 生产环境未部署。
...
- [ ] **任务 4：前端实现后设计走查** —— 对比 Figma 和实际页面，列出差异+优先级。产物：设计走查清单。
...
```
Bun 1.3.14 | PostgreSQL 16.14 + pgvector 0.8.5 | localhost:5432
DB: webtutor / user: dev / password: dev
DeepSeek API: sk-9462... 已配置在 apps/api/.env
