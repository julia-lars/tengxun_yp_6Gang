...
用法:
  ...
  python3 label_all_v3.py --resume           # 断点续跑（已输出文件跳过）
...
def process_file(input_path: str, batch_size: int, resume: bool) -> dict:
    """处理单个 cleaned 文件，返回统计信息。"""
    out_path = output_path_for(input_path)
    ckpt_path = checkpoint_path_for(input_path)

    if resume and os.path.exists(out_path):
        return {"input": input_path, "status": "skipped", "reason": "output_exists"}
    ...
    for start_idx, batch in batches:
        results = label_batch(batch, start_idx)
...
def main():
    ...
    print(f"全量标注开始：{len(files)} 个文件，约 {total_segments} 条 kept 片段")
    print(f"并发: {args.workers} workers, 每批: {args.batch} segments")

    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        futures = {executor.submit(process_file, fp, args.batch, args.resume): fp for fp in files}
